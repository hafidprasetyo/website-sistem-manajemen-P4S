@extends('layouts.masterkonsultan1')
@section('content')

<div class="overflow-x-auto relative shadow-md sm:rounded-lg">
    <table class="w-full text-sm rounded-lg text-left text-gray-500 dark:text-gray-400">
        <caption class="p-5 text-lg font-semibold text-left text-gray-900 bg-white dark:text-white dark:bg-gray-800">
        <form action="{{ route('konsultan.create')}}" method="get">
        @csrf
            Data Konsultan ||<button type="submit" class="ml-3 focus:outline-none text-white bg-primary hover:bg-yellow-400 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-5 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900"> Tambah Konsultan</button></form>
        </caption>
        ...
        @forelse ($konsultans as $konsultan)
                  <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                      ...
                      <td class="text-center text-xxs font-weight-bolder opacity-7 py-3 px-6">{{$konsultan->name}}</td>
                      <td class="text-xxs font-weight-bolder opacity- py-3 px-67"><img class="mx-auto max-w-lg h-auto" width ="100px" height = "300px" src="{{url('')}}/assets/images/{{$konsultan->image}}" class="rounded" alt=""></td>
                      <td class="text-center text-xxs font-weight-bolder opacity- py-3 px-67"><form action="{{ route('konsultan.destroy',['konsultan'=>$konsultan->id]) }}" method="POST" class="font-semibold">
                      @method('DELETE') @csrf 
                      <button type="submit" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Hapus</button></form><br><form action="{{ route('konsultan.edit',['konsultan'=>$konsultan->id]) }}" class="font-semibold">
